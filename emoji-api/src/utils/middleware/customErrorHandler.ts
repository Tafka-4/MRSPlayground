...
const customErrorHandler: ErrorRequestHandler = (err: Error, req: Request, res: Response, next: NextFunction) => {
	...
	if (err instanceof emojiError.EmojiError) {
		if (err instanceof emojiError.EmojiPackageNotFoundError || err instanceof emojiError.EmojiNotFoundError) return res.status(404).json({ message: err.message });
		if (err instanceof emojiError.EmojiUploadFailedError || err instanceof emojiError.EmojiDeleteFailedError || err instanceof emojiError.EmojiInvalidExtensionError) return res.status(418).json({ message: err.message });
		return res.status(400).json({ message: err.message });
	}
