...
redisClient.on('error', async () => {
	if (!isReconnecting && !redisClient.isOpen) {
		isReconnecting = true;
		setTimeout(async () => {
			try { await redisClient.connect(); isReconnecting = false; } catch { isReconnecting = false; }
		}, 5000);
	}
});

const checkRedisConnection = async (): Promise<boolean> => {
	try { if (!redisClient.isOpen) return false; const result = await redisClient.ping(); return result === 'PONG'; } catch { return false; }
};
...
const waitForRedis = async (maxRetries: number = 30, retryInterval: number = 2000): Promise<void> => {
	...
	while (retries < maxRetries) {
		try {
			if (redisClient.isOpen) { try { await redisClient.disconnect(); } catch {} }
