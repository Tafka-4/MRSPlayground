...
export class User {
  ...
  static async create(userData: Omit<IUser, 'userid' | 'isVerified' | 'wroteNovels' | 'favoriteNovels'>): Promise<User> {
    const userid = uuidv4();
  ...
  static async findOneAndUpdate(query: Partial<IUser>, update: Partial<IUser>, options?: { new?: boolean }): Promise<User | null> {
    ...
    Object.entries(update).forEach(([key, value]) => {
      ...
      } else if (key === 'wroteNovels' || key === 'favoriteNovels') {
        updateFields.push(`${key} = ?`);
    ...
    await pool.execute(
      `UPDATE users SET ${updateFields.join(', ')}, updatedAt = CURRENT_TIMESTAMP WHERE userid = ?`,
      values
    );
  ...
  static async deleteOne(query: Partial<IUser>): Promise<void> {
    ...
    await pool.execute('DELETE FROM users WHERE userid = ?', [user.data.userid]);
  }
  ...
  async generateTokens(): Promise<{ accessToken: string, refreshToken: string }> {
    const accessToken = jwt.sign({ userid: this.data.userid }, process.env.JWT_SECRET as string, { expiresIn: "1h" });
    const refreshToken = jwt.sign({ userid: this.data.userid }, process.env.JWT_SECRET as string, { expiresIn: "7d" });
    await redisClient.set(`${this.data.userid}:refreshToken`, refreshToken, { EX: 60 * 60 * 24 });
  ...
  async logout(): Promise<number> {
    const result = await redisClient.del(`${this.data.userid}:refreshToken`);
    return result;
  }

  async refresh(checkRefreshToken: string): Promise<string> {
    const refreshToken = await redisClient.get(`${this.data.userid}:refreshToken`);
    if (!refreshToken) throw new UserNotLoginError("Unauthorized");
    if (refreshToken !== checkRefreshToken) throw new UserTokenVerificationFailedError("Token verification failed");
    const accessToken = jwt.sign({ userid: this.data.userid }, process.env.JWT_SECRET as string, { expiresIn: "1h" });
    return accessToken;
  }
  ...
  async uploadProfileImage(file: MulterFile): Promise<string> {
    ...
    const filename = `${createHmac("sha256", process.env.JWT_SECRET as string).update(this.data.userid).digest("hex")}.${extension}`;
  ...
  get wroteNovels() { return this.data.wroteNovels; }
  get favoriteNovels() { return this.data.favoriteNovels; }
} 