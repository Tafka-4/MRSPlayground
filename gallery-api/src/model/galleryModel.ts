...
const gallerySchema = new mongoose.Schema({
	...
	galleryManager: { type: [String], required: true, default: [] },
...
gallerySchema.pre('deleteOne', { document: true, query: false }, async function (next) {
	if ((this as any).thumbnail) {
		try { fs.unlinkSync((this as any).thumbnail); } catch {}
	}
	await redisClient.del(`gallery:${(this as any).galleryId}:subscribers`);
	next();
});
...
gallerySchema.methods.galleryManagerAdd = async function (manager: string) {
	if (this.galleryAdmin === manager) throw new galleryError.GalleryIsNotAdminError('You are the admin of this gallery');
	if (this.galleryManager.includes(manager)) throw new galleryError.GalleryInteractionFailedError('You are already a manager of this gallery');
...
gallerySchema.methods.galleryManagerDelete = async function (manager: string) {
	if (this.galleryAdmin === manager) throw new galleryError.GalleryIsNotAdminError('You are the admin of this gallery');
	if (!this.galleryManager.includes(manager)) throw new galleryError.GalleryInteractionFailedError('You are not a manager of this gallery');
