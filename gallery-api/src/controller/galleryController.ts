...
export const createGallery = async (req: Request, res: Response) => {
    ...
    if (!galleryId) throw new galleryError.GalleryError('Gallery ID is required');
    if (await Gallery.findOne({ galleryId })) throw new galleryError.GalleryError('Gallery ID already exists');
    await Gallery.create({ galleryId, title, description, galleryAdmin: req.user?.userid });
    res.status(201).json({ success: true, message: 'Gallery created successfully' });
};
...
export const getGalleryList = async (req: Request, res: Response) => {
    ...
    const galleries = await Gallery.find({ title: { $regex: search, $options: 'i' } })
...
export const updateGallery = async (req: Request, res: Response) => {
    ...
    const gallery = await Gallery.findOne({ galleryId });
    if (!gallery) throw new galleryError.GalleryNotFoundError('Gallery not found');
    if (gallery.galleryAdmin !== req.user?.userid) throw new galleryError.GalleryIsNotAdminError('You are not the admin of this gallery');
...
export const subscribeGallery = async (req: Request, res: Response) => {
    ...
    res.status(200).json({ success: true, message: 'Gallery subscribed successfully' });
};
...
export const unsubscribeGallery = async (req: Request, res: Response) => {
    ...
    res.status(200).json({ success: true, message: 'Gallery unsubscribed successfully' });
};
...
export const galleryAdminChange = async (req: Request, res: Response) => {
    ...
    res.status(200).json({ success: true, message: 'Gallery admin changed successfully' });
};
...
export const galleryManagerAdd = async (req: Request, res: Response) => {
    ...
    if (!gallery) throw new galleryError.GalleryNotFoundError('Gallery not found');
    if (gallery.galleryAdmin !== req.user?.userid) throw new galleryError.GalleryIsNotAdminError('You are not the admin of this gallery');
    if (gallery.galleryManager.includes(manager)) throw new galleryError.GalleryInteractionFailedError('You are already a manager of this gallery');
    await gallery.galleryManagerAdd(manager);
    res.status(200).json({ success: true, message: 'Gallery manager added successfully' });
};
...
export const galleryManagerDelete = async (req: Request, res: Response) => {
    ...
    if (!gallery) throw new galleryError.GalleryNotFoundError('Gallery not found');
    if (gallery.galleryAdmin !== req.user?.userid) throw new galleryError.GalleryIsNotAdminError('You are not the admin of this gallery');
    if (!gallery.galleryManager.includes(manager)) throw new galleryError.GalleryInteractionFailedError('You are not a manager of this gallery');
    await gallery.galleryManagerDelete(manager);
    res.status(200).json({ success: true, message: 'Gallery manager deleted successfully' });
};
...
export const galleryThumbnailUpload = async (req: Request, res: Response) => {
    ...
    res.status(200).json({ success: true, message: 'Gallery thumbnail uploaded successfully' });
};
...
export const galleryThumbnailDelete = async (req: Request, res: Response) => {
    ...
    res.status(200).json({ success: true, message: 'Gallery thumbnail deleted successfully' });
};
...
export const addGalleryBlockUser = async (req: Request, res: Response) => {
    ...
    res.status(200).json({ success: true, message: 'Gallery block user added successfully' });
};
...
export const deleteGalleryBlockUser = async (req: Request, res: Response) => {
    ...
    res.status(200).json({ success: true, message: 'Gallery block user deleted successfully' });
};
...
export const addGalleryBlockIP = async (req: Request, res: Response) => {
    ...
    res.status(200).json({ success: true, message: 'Gallery block IP added successfully' });
};
...
export const deleteGalleryBlockIP = async (req: Request, res: Response) => {
    ...
    res.status(200).json({ success: true, message: 'Gallery block IP deleted successfully' });
};
