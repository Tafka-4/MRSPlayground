...
commentSchema.pre('save', async function (next) {
	...
	(this as any).content = await parseEmojiToImgTag((this as any).content, true);
	next();
});
...
commentSchema.methods.like = async function (userId: string): Promise<void> {
	...
	if (!resultLike) throw new commentError.CommentInteractionFailedError('Already liked');
	this.likeCount++;
};
...
commentSchema.methods.dislike = async function (userId: string): Promise<void> {
	...
	if (!resultDislike) throw new commentError.CommentInteractionFailedError('Already disliked');
	this.dislikeCount++;
};
