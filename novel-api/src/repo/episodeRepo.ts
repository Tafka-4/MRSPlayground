...
class PrismaEpisodeRepo implements EpisodeRepo {
  async createEpisode(novelId: string, userId: string, title: string, content: string, authorComment?: string | null): Promise<EpisodeRecord> {
    const result = await prisma.$transaction(async (tx) => {
      const novel = await tx.novel.findUnique({ where: { novelId } });
      if (!novel || novel.author !== userId) throw new Error('Novel not found or not author');
      const nextNo = novel.episodeCount + 1;
      await tx.novel.update({ where: { novelId }, data: { episodeCount: { increment: 1 } } });
      const episode = await tx.episode.create({ data: { episodeId: uuidv4(), episodeNumber: nextNo, novelId, title, content, author: userId, authorComment: authorComment ?? null } });
      return episode;
    });
    return result as any;
  }
  async findById(episodeId: string): Promise<EpisodeRecord | null> {
    return (await prisma.episode.findUnique({ where: { episodeId } })) as any;
  }
  async increaseView(episodeId: string): Promise<void> {
    await prisma.episode.update({ where: { episodeId }, data: { viewCount: { increment: 1 } } });
  }
  async updateIfAuthor(episodeId: string, userId: string, data: any): Promise<EpisodeRecord | null> {
    const ep = await prisma.episode.findUnique({ where: { episodeId } });
    if (!ep) return null;
    const novel = await prisma.novel.findUnique({ where: { novelId: ep.novelId } });
    if (!novel || novel.author !== userId) return null;
    const updated = await prisma.episode.update({ where: { episodeId }, data: { ...data, updatedAt: new Date() } });
    return updated as any;
  }
  async deleteIfAuthorAndLast(episodeId: string, userId: string): Promise<boolean> {
    return await prisma.$transaction(async (tx) => {
      const ep = await tx.episode.findUnique({ where: { episodeId } });
      if (!ep) return false;
      const novel = await tx.novel.findUnique({ where: { novelId: ep.novelId } });
      if (!novel || novel.author !== userId) return false;
      if (novel.episodeCount !== ep.episodeNumber) return false;
      await tx.episode.delete({ where: { episodeId } });
      await tx.novel.update({ where: { novelId: ep.novelId }, data: { episodeCount: { decrement: 1 } } });
      return true;
    });
  }
  async like(episodeId: string, novelId: string, userId: string) {
    ...
    if (!resultLike) throw new Error('Already liked');
    const updated = await prisma.episode.update({ where: { episodeId }, data: { likeCount, dislikeCount } });
    return { likeCount: updated.likeCount, dislikeCount: updated.dislikeCount };
  }
  async dislike(episodeId: string, novelId: string, userId: string) {
    const resultDislike = await redisClient.sAdd(`${novelId}:${episodeId}:dislikes`, userId);
    const resultLike = await redisClient.sRem(`${novelId}:${episodeId}:likes`, userId);
