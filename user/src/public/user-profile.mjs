...
async function loadUserProfile() {
    try {
        const response = await fetch(`/api/v1/users/${targetUserId}`, {
            credentials: 'include',
            headers: {
...
function displayUserProfile(user) { //dummy data
    document.getElementById('loading').style.display = 'none';
    ...
    document.getElementById('posts-count').textContent = Math.floor(Math.random() * 100);
...
document.getElementById('follow-button').addEventListener('click', async () => {
    try {
        ...
        if (isFollowing) {
            ...
            button.innerHTML = '<span class="material-symbols-outlined">person_add</span>팔로우';
            showNotice('팔로우를 취소했습니다.');
            isFollowing = false;
        } else {
            button.classList.add('following');
            icon.textContent = 'person_remove';
            button.innerHTML = '<span class="material-symbols-outlined">person_remove</span>팔로잉';
            showNotice('팔로우했습니다.');
            isFollowing = true;
        }
    } catch (error) {
