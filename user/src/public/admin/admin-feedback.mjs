...
class AdminFeedbackManager {
    ...
    async fetchFeedback(page = 1) {
        ...
        try {
            const response = await api.get('/api/feedback/v1', { query });
    ...
    async viewFeedback(id) {
        try {
            const response = await api.get(`/api/feedback/v1/${id}`);
            if (response && response.success) {
    ...
    async handleUpdate(id) {
        ...
        try {
            const response = await api.put(`/api/feedback/v1/${id}`, { status, admin_notes });
            if (response && response.success) {
