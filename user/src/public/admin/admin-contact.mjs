...
class AdminContactManager {
    ...
    async fetchContacts(page = 1, queryOptions = {}) {
        ...
        try {
            const response = await api.get('/api/contacts/v1', { query });
    ...
    async viewContact(id) {
        try {
            const response = await api.get(`/api/contacts/v1/${id}`);
            if (response && response.success) {
    ...
    async handleUpdate(id) {
        ...
        try {
            const response = await api.put(`/api/contacts/v1/${id}/status`, { status, admin_notes });
            if (response && response.success) {
